...
export default {
    ...
    methods: {
        ...
        searchResult() {
            axios.get('/api/branches?search='+this.search).then((response) => {
                this.branches = response.data.branches;
            }).catch((error) => {
                this.showNoty(error.response.data.message, 'error')
            });
...
<template>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        >
            <RouterLink v-if="$can('branches.create')" :to="{name : 'branches.create' }">
                    ...
                    >
                        ...
                        >
                            <div class="flex items-center justify-center">
                                <RouterLink v-if="$can('branches.update')" :to="{name : 'branches.update', params:{'id' : branch.id} }"
                                            class="flex items-center mr-3">
                                    <Lucide icon="CheckSquare" class="w-4 h-4 mr-1"/>
                                    Edit
                                </RouterLink>
                                <a v-if="$can('branches.remove')"
                                    class="flex items-center text-danger"
                                    href="#"
                                    @click="
                                        (event) => {
                                            event.preventDefault();
                                            setDeleteConfirmationModal(true , branch.id);
                                        }
                                    "
                                >
