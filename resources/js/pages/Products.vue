...
const data = reactive({ products: [], productID: 0, deleteConfirmationModal: false, toastText : '', toastType : 'success', search:'' })
onMounted(() => {
    getProducts();
})

const getProducts = ((url = '/api/products') => {
...
const deleteProduct = (()=> {
    axios.delete('/api/delete-product/' + data.productID).then((response) => {
...
<template>
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        ...
        >
            <RouterLink v-if="$can('products.create')" :to="{name : 'products.create' }">
                    ...
                    >
                            ...
                            <div class="flex">
                                <div class="w-10 h-10 image-fit zoom-in">
                                    <Tippy
                                        ...
                                        :src="product.image !== null ? '/images/products/'+product.image : '/images/categories/profile-2.jpg'"
                                    />
                                </div>
                            </div>
                        </Table.Td>
                        ...
                        >
                            <div class="flex items-center justify-center">
                                <RouterLink v-if="$can('products.update')" :to="{name : 'products.update', params:{'id' : product.id} }"
                                            class="flex items-center mr-3">
                                    <Lucide icon="CheckSquare" class="w-4 h-4 mr-1"/>
                                    Edit
                                </RouterLink>
                                <a v-if="$can('products.remove')"
                                    class="flex items-center text-danger"
                                    href="#"
                                    @click="
                                        (event) => {
                                            event.preventDefault();
                                            setDeleteConfirmationModal(true , product.id);
                                        }
                                    "
                                >
        ...
        >
            <Pagination class="w-full sm:w-auto sm:mr-auto">
                <Pagination.Link @click="getProducts(data.products.first_page_url)">
                    <Lucide icon="ChevronsLeft" class="w-4 h-4" />
                </Pagination.Link>
                <Pagination.Link v-if="data.products.prev_page_url" @click="getProducts(data.products.prev_page_url)">
                    <Lucide icon="ChevronLeft" class="w-4 h-4" />
                </Pagination.Link>
                <Pagination.Link v-if="data.products.current_page - 1 > 1">...</Pagination.Link>
                <Pagination.Link  v-if="data.products.prev_page_url" @click="getProducts(data.products.prev_page_url)">{{data.products.current_page - 1}}</Pagination.Link>
                <Pagination.Link active>{{data.products.current_page}}</Pagination.Link>
                <Pagination.Link  v-if="data.products.next_page_url" @click="getProducts(data.products.next_page_url)">{{data.products.current_page + 1}}</Pagination.Link>
                <Pagination.Link v-if="data.products.last_page - data.products.current_page > 1">...</Pagination.Link>
                <Pagination.Link v-if="data.products.next_page_url" @click="getProducts(data.products.next_page_url)">
                    <Lucide icon="ChevronRight" class="w-4 h-4" />
                </Pagination.Link>
                <Pagination.Link @click="getProducts(data.products.last_page_url)">
                    <Lucide icon="ChevronsRight" class="w-4 h-4" />
                </Pagination.Link>
            </Pagination>
            <label>Page Size</label>
            <select class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 w-20 mt-3 !box sm:mt-0" v-model="data.products.per_page">
